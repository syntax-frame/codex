...
#[test]
fn ssh_environment_routes_exec_through_its_backend() -> anyhow::Result<()> {
    let manager = EnvironmentManager::ssh_with_config(
        "ssh",
        SshEnvironmentConfig {
            connection_key: "test@example.com:22".to_string(),
            agent_key: "node-a".to_string(),
            host: "example.com".to_string(),
            port: 22,
            user: "test".to_string(),
            authentication: SshAuthentication::PrivateKeyPath(
                "/tmp/nonexistent-test-key".to_string(),
            ),
            host_fingerprint: None,
            tmux_mode: SshTmuxMode::Required,
        },
    );
    let environment = manager.default_environment().context("ssh environment")?;
