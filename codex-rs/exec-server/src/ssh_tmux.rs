    ...
    fn bootstrap_command(&self, params: &ExecParams) -> String {
        ...
        format!(
            concat!(
                ...
                "  exit 76\n",
                "else\n",
                "  if ! mkdir \"$root\" 2>/dev/null; then echo AGENTAPP_TMUX_DESCRIPTOR_CAS_CONFLICT >&2; exit 75; fi\n",
                ...
                "  printf '%s\\n' {turn_id} > \"$root/turn-id\"\n",
                "  printf '%s\\n' {call_id} > \"$root/call-id\"\n",
                "  printf '%s\\n' {attempt_generation} > \"$root/attempt-generation\"\n",
                "  printf '%s\\n' {session_id} > \"$root/session-id\"\n",
                "  printf '%s\\n' {tty} > \"$root/tty\"\n",
                "  printf '%s\\n' {acknowledgement_token} > \"$root/acknowledgement-token\"\n",
                "  printf '%s\\n' \"$digest\" > \"$root/digest\"\n",
                "  printf '%s\\n' \"$window\" > \"$root/window\"\n",
                "  printf 'prepared\\n' > \"$root/state\"\n",
                "  create_window=1\n",
