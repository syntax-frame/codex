    ...
    for execution in &request.incomplete_executions {
        ...
        command.push_str(&format!(
            concat!(
                ...
                "    fi\n",
                "  fi\n",
                "  if [ -e \"$root/recovery-required\" ] || [ -L \"$root/recovery-required\" ]; then\n",
                "    recovery_marker=$(cat \"$root/recovery-required\" 2>/dev/null || true)\n",
                "    recovery_generation=${{recovery_marker%%:*}}\n",
                "    recovery_controller=${{recovery_marker#*:}}\n",
                "    current_controller=$(cat \"$root/controller\" 2>/dev/null || true)\n",
                "    current_generation=${{current_controller%%:*}}\n",
                "    current_controller_id=${{current_controller#*:}}\n",
                "    marker_is_stale_and_proven=0\n",
                "    if [ -f \"$root/recovery-required\" ] && [ ! -L \"$root/recovery-required\" ]; then\n",
                "      case \"$recovery_generation\" in ''|*[!0-9]*) ;; *)\n",
                "        case \"$current_generation\" in ''|*[!0-9]*) ;; *)\n",
                "          case \"$recovery_controller\" in ''|*:*|*[!0-9A-Za-z_.-]*) ;; *)\n",
                "            case \"$current_controller_id\" in ''|*:*|*[!0-9A-Za-z_.-]*) ;; *)\n",
                "              if [ \"$recovery_marker\" = \"$recovery_generation:$recovery_controller\" ] && [ \"$current_controller\" = \"$current_generation:$current_controller_id\" ] && [ \"$current_generation\" -gt \"$recovery_generation\" ] && [ \"$terminal_verified_dead\" -eq 1 ]; then\n",
                "                case \"$recovered\" in completed|terminated|expired|recovery-lost|launch-interrupted) marker_is_stale_and_proven=1 ;; esac\n",
                "              fi\n",
                "            esac\n",
                "          esac\n",
                "        esac\n",
                "      esac\n",
                "    fi\n",
                "    recovery_marker_after=$(cat \"$root/recovery-required\" 2>/dev/null || true)\n",
                "    current_controller_after=$(cat \"$root/controller\" 2>/dev/null || true)\n",
                "    [ \"$recovery_marker_after\" = \"$recovery_marker\" ] && [ \"$current_controller_after\" = \"$current_controller\" ] && [ -f \"$root/recovery-required\" ] && [ ! -L \"$root/recovery-required\" ] || marker_is_stale_and_proven=0\n",
                "    [ \"$marker_is_stale_and_proven\" -eq 1 ] || recovered=unknown\n",
                "  fi\n",
