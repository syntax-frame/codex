    ...
    fn bootstrap_command(&self, params: &ExecParams) -> String {
        ...
        format!(
            concat!(
                ...
                "  trap 'exit 130' INT\n",
                "  trap 'exit 143' TERM\n",
                "  if ! (umask 077; mkdir \"$staging\") 2>/dev/null; then echo AGENTAPP_TMUX_DESCRIPTOR_STAGE_CONFLICT >&2; exit 75; fi\n",
                "  owns_staging=1\n",
                "  : > \"$staging/output\"\n",
                "  printf '%s\\n' {owner} > \"$staging/owner\"\n",
                "  printf '%s\\n' {identity} > \"$staging/identity\"\n",
                "  printf '%s\\n' {thread_id} > \"$staging/thread-id\"\n",
                "  printf '%s\\n' {turn_id} > \"$staging/turn-id\"\n",
                "  printf '%s\\n' {call_id} > \"$staging/call-id\"\n",
                "  printf '%s\\n' {attempt_generation} > \"$staging/attempt-generation\"\n",
                "  printf '%s\\n' {session_id} > \"$staging/session-id\"\n",
                "  printf '%s\\n' {tty} > \"$staging/tty\"\n",
                "  printf '%s\\n' {acknowledgement_token} > \"$staging/acknowledgement-token\"\n",
                "  printf '%s\\n' \"$digest\" > \"$staging/digest\"\n",
                "  printf '%s\\n' \"$window\" > \"$staging/window\"\n",
                "  printf 'prepared\\n' > \"$staging/state\"\n",
                "  if [ -e \"$root\" ] || [ -L \"$root\" ]; then echo AGENTAPP_TMUX_DESCRIPTOR_CAS_CONFLICT >&2; exit 75; fi\n",
                "  if ! mv -n \"$staging\" \"$root\" 2>/dev/null || [ -d \"$staging\" ] || [ -d \"$root/$staging_name\" ]; then echo AGENTAPP_TMUX_DESCRIPTOR_CAS_CONFLICT >&2; exit 75; fi\n",
                "  owns_staging=0\n",
